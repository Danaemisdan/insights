                ...
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-6 mb-24">
                        {siteContent.workWithUs.partnerships.list.map((partner, i) => (
                            <div key={i} className={`p-8 text-center rounded-2xl flex flex-col items-center justify-center h-48 transition-all duration-300 ${isVibrant ? 'bg-[#fdfaf2] border-2 border-brand-yellow/10 hover:border-brand-yellow/50 hover:-translate-y-2 hover:shadow-xl' : 'bg-gray-50 border border-gray-100 hover:shadow-lg hover:border-brand-green/30'}`}>
                                <div className={`w-16 h-16 rounded-2xl flex items-center justify-center mb-4 ${isVibrant ? 'bg-brand-yellow/20 text-brand-dark' : 'bg-brand-green/10 text-brand-green'}`}>
                                    {partnerIcons[i]}
                                </div>
                    ...
                    {/* CTA */}
                    <div className={`max-w-4xl mx-auto text-center p-12 ${isVibrant ? 'bg-gradient-to-r from-brand-dark to-brand-green text-white rounded-3xl' : 'bg-gradient-to-r from-brand-green to-brand-dark text-white rounded-2xl'}`}>
                        ...
                        <Link
                            to={`${basePath}/contact`}
                            className={`inline-flex items-center px-8 py-4 rounded-full font-bold uppercase tracking-wider transition-all duration-300 transform hover:-translate-y-1 ${isVibrant ? 'bg-brand-yellow text-brand-dark hover:bg-white' : 'bg-white text-brand-green hover:bg-brand-dark hover:text-white'}`}
                        >
                            Get In Touch

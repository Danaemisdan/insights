...
const InsightsPage = () => {
    const styles = {
        ...
        tagBg: 'bg-white shadow-sm text-brand-green border border-gray-100',
    };
        ...
        <div className="w-full">
                ...
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 py-16 w-full">
                    <div className="flex flex-col md:flex-row justify-between items-center text-center md:text-left gap-8">
                        <div className="max-w-2xl">
                            <span className={`text-sm font-bold uppercase tracking-widest mb-4 block text-brand-orange`}>Knowledge Repository</span>
            ...
            <div className="py-8 bg-white border-b border-gray-100">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    <div className="flex flex-wrap gap-3 justify-center">
                        <span className={`px-5 py-2.5 rounded-full font-bold text-sm cursor-pointer bg-brand-green text-white`}>All</span>
                        {siteContent.insights.categories.map((cat, i) => (
                            <span key={i} className={`px-5 py-2.5 rounded-full font-bold text-sm transition-all cursor-pointer hover:shadow-md bg-gray-50 text-gray-700 hover:bg-brand-green hover:text-white`}>
                                {cat}
                            </span>
                        ))}
                    </div>
                ...
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                        {insightCards.map((insight, idx) => (
                            <article key={idx} className={`rounded-2xl overflow-hidden transition-all duration-300 hover:-translate-y-2 hover:shadow-2xl cursor-pointer bg-white shadow-sm border border-gray-100 hover:border-brand-green/30`}>
                                <div className="h-52 overflow-hidden relative group">
                                    ...
                                    <span className={`absolute top-4 left-4 px-3 py-1 rounded-full text-xs font-bold bg-brand-green text-white`}>{insight.type}</span>
                                </div>
                                <div className="p-6">
                                    ...
                                    <span className={`inline-flex items-center text-sm font-bold text-brand-green`}>
            ...
            <div className={`py-16 bg-gray-50`}>
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
                    <p className={`text-lg font-medium mb-8 text-gray-600`}>Covering the entire spectrum of millet and agri-food knowledge</p>
